...
$sql_select = "SELECT * FROM queue WHERE qu_id = '" . $select_id . "'";
...
<body id="page-top">
    ...
    <div class="my-auto">
        <form class=" w-50 h-50 bg-gradient-info text-white px-5 py-5 coutnform rounded-4 mt-4" id="form_queue">
            <input type="hidden" value="<?php echo $rows_select['qu_id'] ?>" id="qu_id">
            <div class="mb-3">
                <label for="exampleInputEmail1" class="form-label">ຊື່ຄິວ</label>
                <input type="text" class="form-control" id="qu_name" value="<?php echo $rows_select['qu_name'] ?>"
                    aria-describedby="emailHelp">
            </div>

            <div class="mb-3">
                <label for="exampleInputPassword1" class="form-label">ໜາຍເຫດ</label>
                <input type="text" class="form-control" value="<?php $rows_select['qu_remak'] ?>" id="qu_remak">
            </div>
            <button type="submit" class="btn btn-primary ">ບັນທືກ</button>
            <button type="reset" class="btn btn-danger ">ລ້າງ</button>
            <a href="select_queue.php" class="btn btn-success ">ຂໍ້ມູນ</a>
        </form>
    </div>
    ...
    $(document).ready(() => {
        $('#form_queue').submit((e) => {
            e.preventDefault()
            var qu_name = $("#qu_name").val()
            var qu_remak = $("#qu_remak").val()
            var qu_id = $("#qu_id").val()
